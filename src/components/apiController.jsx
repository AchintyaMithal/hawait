...
const API_URL = "http://web-dev.dev.kimo.ai/v1";
//------------------------------------
...
export const getHighlights = () => {
	//const response = await fetch(`${API_URL}/highlights`, { mode: 'no-cors' });
...
export const getCategories = async () => {
	const response = await fetch(`${API_URL}/categories`, { mode: 'no-cors' });
	const data = [{
		"name": "Adventure",
		"activities": [{
...
export const getActivitiesByType = async (type) => {
	const response = await fetch(`${API_URL}/activities/${type}`, { mode: 'no-cors' });
	const data = await response.json();
	return data;
};